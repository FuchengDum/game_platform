...
function Layout() {
  ...
  // 检测是否在游戏页面,使用 in-game class 标记
  const [isGamePage, setIsGamePage] = React.useState(false);

  React.useEffect(() => {
    // 检查 body 是否有 in-game class
    const checkGamePage = () => {
      setIsGamePage(document.body.classList.contains('in-game'));
    };
    ...
    // 监听 class 变化
    const observer = new MutationObserver(checkGamePage);
